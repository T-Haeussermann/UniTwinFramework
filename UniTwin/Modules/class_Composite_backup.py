...
class Composite(Component):
    ...
    def learn(self):
        ...
        try:
                ...
                for item in items_to_relearn:
                    for instance in items_to_relearn[item]:
                        child = self.getChild(items_to_relearn[item][instance]["_id"])
                        if hasattr(child, "_subscribers"):
                            for subscriber in child._subscribers:
                        ...
                        for ch in self._children:
                            if hasattr(ch, "_subscribers"):
                                ...
                                if items_to_relearn[item][instance]["_id"] in ch._subscribers:
                                    # unsubscriber x with method y!
                                    subscriber = items_to_relearn[item][instance]['_id']
                                    subscriberMethod = ch._subscribers[items_to_relearn[item][instance]["_id"]]
                                    # self.unsubscribe(ch, items_to_relearn[item][instance]["_id"], subscriberMethod)
                                    subscriptiontext = self.unsubscribe(ch._id, items_to_relearn[item][instance]["_id"],
                                                                        subscriberMethod)
                                    ch._subscribers.pop(items_to_relearn[item][instance]["_id"])
                                    print(subscriptiontext)
                        self.remove(child)
                    ...
                    for ch in self._children:
                        if hasattr(ch, "_subscribers"):
                            ...
                            if items_to_remove[item][instance]["_id"] in ch._subscribers:
                                ...
                                subscriptiontext = self.unsubscribe(ch._id, items_to_remove[item][instance]["_id"], subscriberMethod)
